...
test.describe("O2C Indent Management", () => {
  ...
  test("User searches and selects dealer from Create Indent modal", { tag: ["@O2C-INDENT-TC-012", "@regression", "@dealer-search", "@iacs-tenant"] }, async ({ Given, page, When, Then, And }) => {
    ...
    await When("I search for dealer by name \"Green Valley\"", null, { page });
    await Then("the dealer list should be filtered", null, { page });
    await And("I should see \"Green Valley\" in the results", null, { page });
    await When("I select the dealer \"Green Valley Agri Center\"", null, { page });
